    ...
    if starts_at
      ...
      if ends_at
        if ends_at.to_date == starts_at.to_date
          if frequency
            desc = "#{l(starts_at, format: DATE_ONLY).titleize}, toutes les #{duration_as_text(frequency)} entre #{l(starts_at, format: TIME_ONLY)} et #{l(ends_at, format: TIME_ONLY)}"
          else
          ...
          if frequency
            desc = "Du #{l(starts_at, format: DATE_ONLY)} au #{l(ends_at, format: DATE_ONLY)}, toutes les #{duration_as_text(frequency)} entre #{l(starts_at, format: TIME_ONLY)} et #{l(ends_at, format: TIME_ONLY)}"
          else
        ...
        if frequency
          desc = "#{l(starts_at, format: DATE_ONLY).titleize}, toutes les #{duration_as_text(frequency)} à partir de #{l(starts_at, format: TIME_ONLY)}"
        else
